...
import { CloudComputing } from "../../../Courses/TechAcademy";

let ArchitectdesignInfo = () => {
  return (
    <div>
      <CourseinfoHero coursename="Microsoft Azure Architect Design" />
...
let ArchitectdesignRelated = () => {
  ...
  // 	techacademy.cloudcomputing
  // );

  // const handleFilter = (id) => {
  // 	const relatedC = relatedCourses.filter(
  // 		(relatedCourse) => relatedCourse.id !== id
  // 	);
  // 	setRelatedCourses(relatedC);
  // };
  return (
      ...
      <div className="row">
        {/* <CloudComputing
					relatedCourses={relatedCourses.filter(
						(relatedCourse) => relatedCourse.id !== 2
					)}
				/> */}
        {techacademy.cloudcomputing.map((singlecloud) => {
          if (singlecloud.id !== 2) {
            return (
              <div className="col-lg-4 col-md-6 col-sm-12" key={singlecloud.id}>
